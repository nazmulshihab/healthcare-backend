...
// // 5. Create appointment (UPDATED with validations)
// export const createAppointment = async (req, res) => {
//   try {
//     const { patientID, doctorID, appointment_date, appointment_time } = req.body;
...
//     if (!schedule) return res.status(400).json({ message: `No schedule found for ${dayName}` });

//     // Get doctor's slots
//     const docSchedule = await models.doc_sc.findOne({
//       where: { doc_id: doctorID, sc_id: schedule.id },
//     });
//     if (!docSchedule) return res.status(400).json({ message: "Doctor is not available on this day" });
...
//       return res.status(400).json({ message: "No slots available for this doctor on this day" });
//     }
...
//     const [updated] = await models.appointments.update(
//       { status },
